...
describe('ConnectomeParser', () => {
    ...
    describe('parseConnectome', () => {
        ...
        it('should correctly create connections between neurons', () => {
            ...
            expect(n1?.connections).toHaveLength(1);
            expect(n1?.connections[0]).toEqual({
                target: n2,
                weight: 0.5,
                type: SynapseType.Chemical
            });

            expect(n2?.connections).toHaveLength(1);
            expect(n2?.connections[0]).toEqual({
                target: n3,
                weight: 0.8,
                type: SynapseType.Chemical
            });

            expect(n3?.connections).toHaveLength(0);
        });
        ...
        it('should handle missing source or target neurons gracefully', () => {
            ...
            expect(n1?.connections).toHaveLength(0);
        });
    });
